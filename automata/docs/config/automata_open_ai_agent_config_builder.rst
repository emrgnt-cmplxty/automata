...
``OpenAIAutomataAgentConfigBuilder`` is a builder class for constructing
...
``OpenAIAutomataAgentConfigBuilder`` extends the base class
...
provides classmethods for creating an ``OpenAIAutomataAgentConfig``
...
``OpenAIAutomataAgentConfigBuilder`` with custom configuration options:
...
.. code:: python
   ...
   builder = OpenAIAutomataAgentConfigBuilder.from_name(config_name)
...
-  Can the ``OpenAIAutomataAgentConfigBuilder`` be made even more
