...
def configure(ctx, *args, **kwargs) -> None:
    ...
    """
    This command uses click to create an interactive CLI command for configuring envirnoment variables.
    Upon running the automata configure command, the .env is created if it doesn't already exist and the user is prompted to enter the values for their environment variables.
    This ensures that the user does not have to manually edit the .env file.
    """
...
def run_eval(ctx, *args, **kwargs) -> None:
    ...
    poetry run automata run-eval --evals-filepath=automata/config/eval/primary_agent_payload.json --model="gpt-4" --toolkits="context-oracle" --log-level=DEBUG --max-iterations=3
