...
def run_agent(ctx: click.Context, *args, **kwargs) -> None:
    ...
    Ex:
    poetry run automata run-agent --model="gpt-4" --toolkits="agent-search,py-reader" --log-level=DEBUG --max-iterations=20 --instructions="Return a markdown snippet containing python code which creates an instance of symbol search  and stores it into a variable `x` when executed."
    """
