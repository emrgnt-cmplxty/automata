...
class PyContextHandler:
    ...
    def construct_symbol_context(
        ...
        base_context = (
            f"{self.retriever.process_symbol(symbol, primary_active_components)}"
        )
    ...
    def get_top_n_symbol_rank_matches(self, symbol: "Symbol") -> List["Symbol"]:
        ...
        return [
            ele[0]
            for ele in symbol_rank_results[: self.config.top_n_symbol_rank_matches]
        ]
    ...
    def get_top_n_symbol_dependencies(self, symbol: "Symbol") -> List["Symbol"]:
        ...
        """
        symbol_dependencies = self.symbol_search.symbol_graph.get_symbol_dependencies(
            symbol
        )
