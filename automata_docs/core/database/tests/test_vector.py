...
from automata_docs.core.symbol.symbol_types import Embedding


def test_init_vector(temp_output_filename):
    JSONVectorDB(temp_output_filename)


def test_add_symbol(temp_output_filename, symbols):
    vector_db = JSONVectorDB(temp_output_filename)
    symbol = symbols[0]
    embedded_symbol = Embedding(symbol, [1, 2, 3])
...
def test_add_symbols(temp_output_filename, symbols):
    vector_db = JSONVectorDB(temp_output_filename)
    embedded_symbol_0 = Embedding(symbols[0], [1, 2, 3])
    vector_db.add(embedded_symbol_0)
    embedded_symbol_1 = Embedding(symbols[1], [1, 2, 3, 4])
...
def test_load(temp_output_filename, symbols):
    ...
    vector_db_2 = JSONVectorDB(temp_output_filename)
