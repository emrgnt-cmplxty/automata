...
def python_writer_tool_builder(tmpdir):
    ...
    return PyCodeWriterToolkitBuilder(py_writer=py_writer)
...
def test_bootstrap_module_with_new_function(python_writer_tool_builder):
    ...
    absolute_path = os.sep.join(
        os.path.abspath(current_file).split(os.sep)[:-1]
    )
...
def test_extend_module_with_new_function(python_writer_tool_builder):
    ...
    with open(
        os.path.join(absolute_path, "..", "sample_modules", "sample.py"),
        "r",
        encoding="utf-8",
    ) as f:
        prev_text = f.read()
...
def test_extend_module_with_documented_new_function(
...
# that has documentation and type hints, e.g. "f(x) -> int;    return x + 1"
@pytest.mark.skip(
    reason="TODO: I don't really understand what this test is trying to do"
)
...
@pytest.mark.skip(
    reason="TODO: something here is clearly off wrt to the files and directories"
)
...
class PythonAgentToolkit:
    ...
    tools = python_writer_tool_builder.build()
    code_writer = tools[0]
    code_writer.function(
        ("sample_modules.python_agent_tool_builder", None, module_str)
    )
    with open(
        "./sample_modules/python_agent_tool_builder.py", "r", encoding="utf-8"
    ) as f:
        new_sample_text = f.read()
