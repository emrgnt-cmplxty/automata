...
# FIXME: This test is becoming too complicated, need to refactor and make sure it works as expected
...
# @patch('automata.cli.scripts.run_doc_embedding.dependency_factory.set_overrides')
# @patch('os.path.join', return_value="mock_directory")
# @patch('builtins.open', new_callable=mock_open, read_data=b"binary data")
# def test_initialize_providers(mock_open_file, mock_path_join, mock_set_overrides, mock_provider, mock_db, mock_symbol_graph, mock_initialize):
#     symbol_handler, filtered_symbols = initialize_providers(embedding_level=2, project_name="test")
